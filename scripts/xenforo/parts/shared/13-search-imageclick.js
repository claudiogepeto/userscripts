    ...
    function buildSearchResultsPanel() {
        ...
        // Dropdown Multiselect de badges para filtragem instantânea nos resultados da busca
        const searchRows = Array.from(document.querySelectorAll('.block-container .block-row, .block-container .contentRow, .structItemContainer .structItem--thread, .block-row, .contentRow'))
            .filter((row, idx, arr) => !row.closest('#smg-rs-panel, .smg-rs-panel') && !arr.some(other => other !== row && other.contains(row)));

        if (searchRows.length > 0) {
            const badgeMap = new Map();
            searchRows.forEach(row => {
                const bList = (typeof extractRowBadges === 'function') ? extractRowBadges(row) : [];
                bList.forEach(b => {
                    const k = b.name.toLowerCase();
                    if (!badgeMap.has(k)) {
                        badgeMap.set(k, { name: b.name, type: b.type, className: b.className, style: b.style, count: 0 });
                    }
            ...
            if (badgeMap.size > 0) {
                ...
                const sortedBadges = Array.from(badgeMap.values()).sort((a, b) => b.count - a.count);

                const applySearchBadgeFilter = () => {
                    const normalizeTag = s => (s || '').toLowerCase().replace(/^[#\s]+|[#\s]+$/g, '').trim();
                    const activeSelectedTags = new Set(Array.from(activeSearchBadges).map(normalizeTag));
                    searchRows.forEach(row => {
                        const bList = (typeof extractRowBadges === 'function') ? extractRowBadges(row) : [];
                        const threadTags = bList.map(b => normalizeTag(b.name));
                        const match = activeSelectedTags.size === 0 || threadTags.some(t => activeSelectedTags.has(t));
                        row.style.display = match ? '' : 'none';
                    });
                };

                if (typeof createBadgeMultiselect === 'function') {
                    const ms = createBadgeMultiselect({
                        badges: sortedBadges,
                        activeBadges: activeSearchBadges,
                        placeholder: i18n('Search tags…'),
                        buttonLabel: i18n('Tags'),
                        onChange: () => applySearchBadgeFilter()
                    });
                    if (ms) {
                        const msEl = ms.container || ms;
                        const toolbar = panel.querySelector('.smg-search-toolbar');
                        if (toolbar && orderBtn) toolbar.insertBefore(msEl, orderBtn);
                        else panel.appendChild(msEl);
                    }
                }
            }
        }
